...
if use_jpg_coords:
    ...
    if uploaded_jpg:
        try:   # Icahnged
            # Save the uploaded JPG file to a temporary location
            jpg_path = os.path.join(st._config.get_option("server.headless.tmp_file_manager"), uploaded_jpg.name)
...
if st.button("Find Nearby Places"):
    if not coordinates_extracted and not (latitude and longitude):
        ...
        if google_api_key:
            nearby_places = find_nearby_places(latitude, longitude, place_type, radius, google_api_key)
